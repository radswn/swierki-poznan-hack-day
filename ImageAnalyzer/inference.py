...
class Inference:
    def __init__(self, satelite: Satelite, inference_function=baseline_inference,
    ...
    def infer_bounding_box(self, coordinates, save=False):
        ...
        for coords, img in self.satelite.iterate_over_box(coordinates, save=False):
            inferenced_image = self.inference_function(
                img)  # image returned from the model (image segmentation, bounding box etc)
